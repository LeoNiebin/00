...
excerpt: >-
  ...
  Include a `collection` object in the request body that contains the following
  required properties:
  ...
  **Note:**
  ...
  - For a complete list of available property values for this endpoint, use the
  following references available in the [collection.json schema
  ...
  Use caution when using this endpoint. The system will replace the existing
  collection with the values passed in the request body.
api:
  file: 'API Documentation #reference.postman_collection.json'
  operationId: put_collections-collectionid
hidden: false
---